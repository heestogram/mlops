...
def predict(dataset: PredictionInput):
    ...
    # 요일 인코딩
    weekday = get_weekday_code(dataset.date)

    # drop_1 ~ drop_10 변수 만들기
    drop_features = get_drop_features(drop_df, dataset.station, dataset.direction)

    # train_no 추정
    train_no = estimate_train_no(final_data, dataset.station, dataset.time, direction)

    # 인원변화, 열차내인원 모수에 근거하여 랜덤샘플링
    onboard_person, delta_person = sample_passenger_info(
        final_data,
        dataset.station,
        dataset.time,
        direction,
